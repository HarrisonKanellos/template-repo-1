# Template Repository 1

## Includes the following setup configurations:
- `npm init -y` to create package.json file
- npm scripts: build, dev
- Core webpack packages: webpack, webpack-cli, webpack-dev-server, webpack-merge
- Webpack plugins: css-loader, html-loader, html-webpack-plugin, style-loader
- Webpack configuration files using webpack merge: webpack.common.js, webpack.dev.js, webpack.prod.js
- Default code files part of webpack configuration: index.js. template.html
- .gitignore: node_modules, dist